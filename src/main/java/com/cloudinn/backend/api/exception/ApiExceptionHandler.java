package com.cloudinn.backend.api.exception;

import com.cloudinn.backend.domain.exception.BusinessLogicException;
import com.cloudinn.backend.domain.exception.EntityNotFoundException;
...
public class ApiExceptionHandler extends ResponseEntityExceptionHandler {
    ...
    public ResponseEntity<Object> handleNotificationException(NotificationException ex, WebRequest request) {
        ...
        final var status = HttpStatus.INTERNAL_SERVER_ERROR;
        final var problem = createProblemBuilder(status,
                ProblemType.ERRO_DE_SISTEMA, ex.getMessage()).build();
        return handleExceptionInternal(ex, problem, new HttpHeaders(), status, request);
    }
